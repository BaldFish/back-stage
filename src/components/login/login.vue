    ...
    methods: {
      ...
      register: function () {
        ...
        $("input[type$='text'],input[type$='password']").each(function (n) {
          if ($(this).val() == "") {
            that.$layer.alert(str += "" + $(this).attr("data-name") + "不能为空！\r\n", {
              shadeClose: false,
              title: '提示框',
            })
            num++;
        ...
        if (num > 0) {
          ...
          var registerInfo = "username=" + that.formLabelAlign.name + "&password=" + that.formLabelAlign.pass + "&captcha_id=" + that.formLabelAlign.pngId + "&captcha_number=" + that.formLabelAlign.code;
          axios({
            method: "post",
            url: registerURL,
